import { asn1, pkcs12, pkcs7, pki, util } from 'node-forge';
...
export class SignPdf {
    ...
    private byteRangePlaceholder = DEFAULT_BYTE_RANGE_PLACEHOLDER;
    ...
    public sign(
        ...
        const forgeCert = util.createBuffer(p12Buffer.toString('binary'));
        ...
        })[pki.oids.certBag] as pkcs12.Bag[];
        const keyBags = p12.getBags({
            bagType: pki.oids.pkcs8ShroudedKeyBag,
        })[pki.oids.pkcs8ShroudedKeyBag] as pkcs12.Bag[];
        ...
        certBags.forEach(bag => {
            const publicKey = (bag.cert as pki.Certificate).publicKey as pki.rsa.PublicKey;

            p7.addCertificate(bag.cert as pki.Certificate);
