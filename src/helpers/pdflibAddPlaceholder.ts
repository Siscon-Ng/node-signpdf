...
export async function pdflibAddPlaceholder(opts: PdflibAddPlaceholderOpts): Promise<Buffer> {
    ...
    try {
        ...
        if (err.message.includes('encrypted')) {
            throw new SignPdfError('Problem loading PDF, PDF encrypted', ERROR_TYPE_PARSE);
        } else {
            throw err;
        }
    }
    ...
    signatureDictMap.set(PDFName.of('Contents'), PDFHexString.fromText('0'.repeat(signatureLength)));
    ...
    let pdfDocBytes = await PDFWriter.forContext(pdfDoc.context, 1).serializeToBuffer();
    ...
    actualByteRange += '  ';
    const byteRangePos = pdfDocBytes.indexOf(byteRangeString as any);
    if (byteRangePos !== -1) {
