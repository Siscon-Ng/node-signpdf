 ...
 */
const createPdf = (params?: any) => new Promise<Buffer>((resolve) => {
    const requestParams = {
        placeholder: {},
        text: 'node-signpdf',
...
describe('Test signing', () => {
    it('expects PDF to be Buffer', () => {
        try {
            signer.sign('non-buffer' as any as Buffer, Buffer.from(''));
    ...
    it('expects P12 certificate to be Buffer', () => {
        try {
            signer.sign(Buffer.from(''), 'non-buffer' as any as Buffer);
            expect('here').not.toBe('here');
        } catch (e) {
            expect(e instanceof SignPdfError).toBe(true);
            expect(e.type).toBe(ERROR_TYPE_INPUT);
    ...
    it('signs detached', async () => {
        ...
        signer.sign(pdfBuffer, p12Buffer);
        const signature2 = signer.lastSignature as string;
    ...
    it('errors when no matching certificate is found in bags', async () => {
        ...
        const originalPkcs12FromAsn1 = pkcs12.pkcs12FromAsn1;
        let p12Instance: any;
        pkcs12.pkcs12FromAsn1 = (...params: any[]) => {
            const [obj, ...otherParams] = params;
            // This instance will be used for all non-mocked code.
            p12Instance = originalPkcs12FromAsn1(obj, ...otherParams);

            return {
                ...p12Instance,
                getBags: ({ bagType }: { bagType: any }) => {
                    if (bagType === pki.oids.certBag) {
                        // Only mock this case.
        ...
        try {
            ...
            pkcs12.pkcs12FromAsn1 = originalPkcs12FromAsn1;
        }
    });
});
